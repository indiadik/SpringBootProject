...
<tr><th>LOCATION</th><th>SALARY</th><th>TYPE</th><th>QUALIFICATION</th><th>APPLY</th></tr>
<c:if test="${!empty list1 }">
<c:forEach items ="${list1 }" var="l">
<tr>
<td> <c:out value="${l.loc }"/></td>
<td> <c:out value="${l.sal }"/></td>
<td> <c:out value="${l.type }"/></td>
<td> <c:out value="${l.qual }"/></td>
<td><a href="apply-wdg">APPLY</a>
</tr>
</c:forEach>
</c:if>
</table>
<c:if test="${back }">
<a href="design?index=${current-1 }" >BACK</a>
</c:if>
<c:if test="${next }">
<a href="design?index=${current+1 }" />NEXT</a>
</c:if>
